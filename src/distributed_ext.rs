...
use crate::{
    ChannelResolver, DistributedConfig, WorkUnitFeed, WorkUnitFeedProvider, WorkerResolver,
...
pub trait DistributedGetterExt: Sized {
    ...
    /// [RouteTasksHandler::route_tasks] to resolve available worker URLs.
...
impl DistributedExt for SessionConfig {
    ...
    fn set_distributed_desired_task_count_handler<H: DesiredTaskCountHandler>(&mut self, h: H) {
        DesiredTaskCountHandlers::register_custom(self, Arc::new(h))
    }

    fn set_distributed_scale_up_leaf_node_handler<H: ScaleUpLeafNodeHandler>(&mut self, h: H) {
        ScaleUpLeafNodeHandlers::register_custom(self, Arc::new(h));
    }

    fn set_distributed_route_tasks_handler<H: RouteTasksHandler>(&mut self, h: H) {
        RouteTasksHandlers::register_custom(self, Arc::new(h));
    }
